...
class InputBox:
    """
    Класс для создания интерактивного поля ввода в интерфейсе PyGame.
    
    Обеспечивает функциональность текстового ввода с валидацией,
    выделением активного поля и обработкой событий клавиатуры и мыши.
    """
    
    def __init__(self, x, y, w, h, text='', label='', is_numeric=True):
        """
        ...
        -----------
        x, y : int
            Координаты верхнего левого угла поля ввода
        w, h : int
            Ширина и высота поля ввода в пикселях
        text : str, optional
            Начальный текст в поле (по умолчанию пустая строка)
        label : str, optional
            Текст метки над полем ввода (по умолчанию пустая строка)
        is_numeric : bool, optional
            Флаг, указывающий что поле должно принимать только числовые значения 
            (по умолчанию True)
        """
        # Прямоугольник для основного поля ввода
        self.rect = pygame.Rect(x, y, w, h)
        # Прямоугольник для метки над полем ввода (на 25 пикселей выше)
        self.label_rect = pygame.Rect(x, y - 25, w, 25)
        
        # Цвета для различных состояний поля
        ...
        # Текстовое содержимое поля
        self.text = text
        # Текст метки (пояснение к полю ввода)
        self.label = label
        
        # Шрифты для отображения текста
        self.font = pygame.font.Font(None, 32)        # Шрифт для содержимого поля
        self.label_font = pygame.font.Font(None, 28)  # Шрифт для метки
        
        # Флаг активности поля (True если поле выбрано для ввода)
        self.active = False
        
        # Флаг числового ввода (ограничение на ввод только чисел)
        self.is_numeric = is_numeric
        
        # Флаг для чисел с плавающей точкой (определяется по наличию точки в начальном тексте)
        self.is_float = '.' in text
    
    def handle_event(self, event):
        """
        Обработка событий PyGame для поля ввода.
        
        Параметры:
        -----------
        event : pygame.event.Event
            Событие PyGame для обработки
            
        Возвращает:
        -----------
        bool
            True если была нажата клавиша Enter, иначе False
        """
        # Обработка клика мышью
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Проверка, был ли клик внутри прямоугольника поля ввода
            if self.rect.collidepoint(event.pos):
                # Переключение активности поля (активировать/деактивировать)
                self.active = not self.active
            else:
                # Деактивация поля при клике вне его области
                self.active = False
            
            # Изменение цвета в зависимости от активности
            self.color = self.color_active if self.active else self.color_inactive
        
        # Обработка нажатия клавиш (только если поле активно)
        if event.type == pygame.KEYDOWN:
            if self.active:
                # Нажатие Enter - завершение ввода
                if event.key == pygame.K_RETURN:
                    return True  # Сигнал о завершении ввода
                
                # Нажатие Backspace - удаление последнего символа
                elif event.key == pygame.K_BACKSPACE:
                    self.text = self.text[:-1]  # Удаление последнего символа
                
                else:
                    # Добавление нового символа с валидацией
                    char = event.unicode  # Получение символа из события
                    
                    if self.is_numeric:
                        # ВАЛИДАЦИЯ ДЛЯ ЧИСЛОВЫХ ПОЛЕЙ:
                        # Разрешаем:
                        # 1. Цифры (0-9)
                        # 2. Точку (только одну и только если поле предназначено для float)
                        # 3. Минус (только в начале числа)
                        if (char.isdigit() or  # Цифры всегда разрешены
                            (char == '.' and '.' not in self.text and self.is_float) or  # Одна точка для float
                            (char == '-' and len(self.text) == 0)):  # Минус только в начале
                            self.text += char  # Добавление валидного символа
                    
                    else:
                        # ВАЛИДАЦИЯ ДЛЯ ТЕКСТОВЫХ ПОЛЕЙ:
                        # Разрешаем буквы, цифры, запятые, скобки и пробелы
                        if char.isalnum() or char in '(), ':
                            self.text += char  # Добавление валидного символа
        
        return False  # Ввод не завершен
    
    def draw(self, screen):
        """
        Отрисовка поля ввода и его метки на экране.
        ...
        """
        # Отрисовка метки над полем ввода
        label_surface = self.label_font.render(self.label, True, (255, 255, 255))  # Белый текст
        screen.blit(label_surface, (self.label_rect.x, self.label_rect.y))  # Позиционирование метки
        
        # Отрисовка прямоугольника поля ввода с рамкой толщиной 2 пикселя
        pygame.draw.rect(screen, self.color, self.rect, 2)
        
        # Отрисовка текста внутри поля ввода
        text_surface = self.font.render(self.text, True, (255, 255, 255))  # Белый текст
        # Позиционирование текста с отступом 5 пикселей от левого и верхнего края
        screen.blit(text_surface, (self.rect.x + 5, self.rect.y + 5))
    
    def get_value(self):
        """
        Получение значения из поля ввода с преобразованием к нужному типу.
        
        Возвращает:
        -----------
        int, float, str или None
            Преобразованное значение или None при ошибке преобразования
        """
        if self.is_numeric:
            if self.is_float:
                try:
                    return float(self.text)  # Преобразование к float
                except ValueError:
                    return None  # Возврат None при некорректном формате
            else:
                try:
                    return int(self.text)  # Преобразование к int
                except ValueError:
                    return None  # Возврат None при некорректном формате
        
        # Для нечисловых полей возвращаем текст как есть
        return self.text


class ParameterInputWindow:
    """
    Окно для ввода параметров калибровки стереокамеры.
    
    Предоставляет пользовательский интерфейс для настройки:
    - Размера шахматной доски (количество пересечений)
    - Размера квадрата доски в сантиметрах
    """
    ...
    def __init__(self):
        ...
        # Создание окна размером 600x450 пикселей
        self.screen = pygame.display.set_mode((600, 450))
        pygame.display.set_caption("Параметры калибровки")  # Заголовок окна
        
        # Часы для контроля частоты кадров
        self.clock = pygame.time.Clock()
        
        # Флаг работы окна (True пока окно открыто)
        self.running = True
        
        # Создание полей ввода с значениями по умолчанию из глобальных констант
        self.chessboard_width = InputBox(
            200, 100, 150, 40,  # x, y, width, height
            str(CHESSBOARD_SIZE[0]),  # Начальное значение из константы
            "Ширина шахматной доски (пересечений):",  # Метка
            is_numeric=True  # Только числовой ввод
        )
        
        self.chessboard_height = InputBox(
            200, 180, 150, 40,
            str(CHESSBOARD_SIZE[1]),  # Начальное значение из константы
            "Высота шахматной доски (пересечений):",
            is_numeric=True
        )
        
        self.square_size = InputBox(
            200, 260, 150, 40,
            str(SQUARE_SIZE),  # Начальное значение из константы
            "Размер квадрата (см):",
            is_numeric=True
        )
        
        # Список всех полей ввода для удобной обработки
        self.input_boxes = [self.chessboard_width, self.chessboard_height, self.square_size]
        
        # Создание прямоугольников для кнопок
        self.button_ok = pygame.Rect(150, 350, 150, 50)      # Кнопка "OK"
        self.button_cancel = pygame.Rect(350, 350, 150, 50)  # Кнопка "Отмена"
        
        # Шрифты для интерфейса
        self.font = pygame.font.Font(None, 36)      # Основной шрифт
        self.info_font = pygame.font.Font(None, 24) # Шрифт для информационного текста
        
        # Результат ввода (заполняется при нажатии OK)
        self.result = None  # Формат: (chessboard_size, square_size) или None при отмене
    
    def run(self):
        """
        Основной цикл окна ввода параметров.
        
        Возвращает:
        -----------
        tuple or None
            Кортеж (chessboard_size, square_size) при успешном вводе,
            None при отмене операции
        """
        while self.running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    self.running = False
                    self.result = None  # Отмена операции
                ...
                # Обработка событий для каждого поля ввода
                for box in self.input_boxes:
                    if box.handle_event(event):
                        # Если в поле ввода нажат Enter - принять параметры
                        self.accept_parameters()
                
                # Обработка кликов мыши по кнопкам
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if self.button_ok.collidepoint(event.pos):
                        self.accept_parameters()  # Клик по кнопке OK
                    elif self.button_cancel.collidepoint(event.pos):
                        self.running = False
                        self.result = None  # Клик по кнопке Отмена
        ...
        # Завершение работы PyGame и возврат результата
        pygame.quit()
        return self.result
    
    def accept_parameters(self):
        """
        Проверка и принятие введенных параметров.
        
        Выполняет валидацию введенных значений и устанавливает результат
        при корректных данных.
        """
        try:
            # Получение значений из полей ввода
            width = self.chessboard_width.get_value()
            height = self.chessboard_height.get_value()
            square_size = self.square_size.get_value()
            
            # Проверка, что все значения были корректно преобразованы
            if width is None or height is None or square_size is None:
            ...
            # Формирование кортежа размера шахматной доски
            chessboard_size = (int(width), int(height))
            
            # Преобразование размера квадрата в float (допускает десятичные значения)
            square_size = float(square_size)
            
            # Установка результата и завершение работы окна
            self.result = (chessboard_size, square_size)
            self.running = False
            
        except ValueError as e:
            # Обработка ошибок валидации
            print(f"Ошибка ввода: {e}")
            # В будущем можно добавить отображение ошибки в интерфейсе
    
    def draw(self):
        """Отрисовка всех элементов интерфейса окна."""
        # Заполнение фона темно-синим цветом
        self.screen.fill((30, 30, 50))
        
        # Заголовок окна
        title = self.font.render("Введите параметры калибровки", True, (255, 255, 0))
        self.screen.blit(title, (150, 20))
        
        # Информационные строки (пояснения для пользователя)
        info_lines = [
            "Эти параметры используются для обнаружения",
            "шахматной доски на изображениях.",
            "",
            "Укажите реальные размеры калибровочной доски."
        ]
        
        # Отрисовка каждой информационной строки
        for i, line in enumerate(info_lines):
            info_text = self.info_font.render(line, True, (200, 200, 200))
            self.screen.blit(info_text, (50, 50 + i * 20))
        ...
        # Отрисовка кнопок
        # Кнопка OK (зеленая)
        pygame.draw.rect(self.screen, (0, 180, 0), self.button_ok)
        # Кнопка Отмена (красная)
        pygame.draw.rect(self.screen, (180, 0, 0), self.button_cancel)
        
        # Текст на кнопках
        ok_text = self.font.render("OK", True, (255, 255, 255))
        cancel_text = self.font.render("Отмена", True, (255, 255, 255))
        
        # Позиционирование текста по центру кнопок
        self.screen.blit(ok_text, (self.button_ok.x + 55, self.button_ok.y + 10))
        self.screen.blit(cancel_text, (self.button_cancel.x + 35, self.button_cancel.y + 10))
        
        # Подсказки для пользователя
        hint1 = self.info_font.render("Нажмите TAB для переключения между полями", True, (150, 150, 150))
        hint2 = self.info_font.render("ESC для отмены, ENTER для подтверждения", True, (150, 150, 150))
        
        # Отрисовка подсказок
        self.screen.blit(hint1, (150, 320))
        self.screen.blit(hint2, (150, 340))
...
def main():
    ...
    # Шаг 1: Ввод параметров калибровки через GUI
    print("\nШАГ 1: Ввод параметров калибровки...")
    
    param_window = ParameterInputWindow()
    parameters = param_window.run()
    
    if parameters is None:
        print("Ввод параметров отменен. Выход из программы.")
        sys.exit(0)
    
    # Обновляем глобальные переменные
    CHESSBOARD_SIZE, SQUARE_SIZE = parameters
    
    print(f"\nУстановлены параметры калибровки:")
    print(f"  Размер шахматной доски: {CHESSBOARD_SIZE[0]} x {CHESSBOARD_SIZE[1]} пересечений")
    print(f"  Размер квадрата: {SQUARE_SIZE} см")
