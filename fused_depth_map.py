...
def draw_depth_legend_meters(img, stereo_calib, fused_depth_uint8, position='right'):
    ...
    Назначение функции:
    -------------------
    Добавляет вертикальную цветовую легенду к карте глубины, показывающую:
    - Расстояния до объектов в метрах (динамически для каждого кадра)
    - Цветовое соответствие (тёплые цвета = близко, холодные = далеко)
    - Технические параметры для отладки (диспаритет, базис, калибровка)
    
    Принцип работы:
    --------------
    1. Извлекает параметры калибровки из stereo_calib (фокус, базис)
    2. Определяет текущий диапазон глубин в кадре (мин/макс диспаритет)
    3. Конвертирует диспаритет в метры по формуле триангуляции
    4. Применяет калибровочный коэффициент для коррекции систематических ошибок
    5. Создаёт цветовой градиент через COLORMAP_JET (как основная карта)
    6. Накладывает текстовые метки с расстояниями и параметрами
    
    ПАРАМЕТРЫ:
    ----------
    img : np.ndarray
        Изображение карты глубины (BGR, COLORMAP_JET) для модификации.
        Изменяется in-place, поэтому рекомендуется передавать копию.
    
    stereo_calib : dict
        Словарь с параметрами стереокалибровки из load_stereo_calibration_with_scaling().
        Обязательные ключи:
        - 'focal_length' : фокусное расстояние в пикселях (масштабированное)
        - 'baseline' : базис (расстояние между камерами) в сантиметрах
    
    fused_depth_uint8 : np.ndarray
        Итоговая карта глубины после фьюжна в диапазоне [0, 255].
        0 = невалидно/далеко, 255 = максимально близко.
        Используется для определения текущего диапазона глубин в кадре.
    
    position : str, optional
        Позиция легенды на изображении.
        По умолчанию 'right' (вертикальный столбик справа).
        Поддерживаемые значения: 'right', 'bottom'.
    
    ВОЗВРАЩАЕТ:
    ----------
    np.ndarray
        Изображение с наложенной легендой (то же самое img, модифицированное).
    """
    
    # ========================================================================
    # БЛОК 1: ПОЛУЧЕНИЕ РАЗМЕРОВ ИСХОДНОГО ИЗОБРАЖЕНИЯ
    # ========================================================================
    # Определяем высоту и ширину входного изображения для расчёта позиции легенды
    h, w = img.shape[:2]
    
    # ========================================================================
    # БЛОК 2: ИЗВЛЕЧЕНИЕ ПАРАМЕТРОВ СТЕРЕОКАЛИБРОВКИ
    # ========================================================================
    # Получаем фокусное расстояние из матрицы камеры (в пикселях)
    # Значение уже масштабировано под PROCESSING_SCALE в load_stereo_calibration_with_scaling()
    focal_length = stereo_calib.get('focal_length', 500)
    
    # Получаем базис (расстояние между оптическими центрами камер)
    # ВАЖНО: это значение в САНТИМЕТРАХ, т.к. в stereo_calibration.py square_size задан в см
    # Вычисляется как норма вектора трансляции T из стереокалибровки
    baseline_cm = stereo_calib.get('baseline', 6.0)
    
    # ========================================================================
    # БЛОК 3: КОНВЕРТАЦИЯ БАЗИСА В МЕТРЫ
    # ========================================================================
    # Формула триангуляции требует базис в метрах, а калибровка хранит в сантиметрах
    # Деление на 100 конвертирует сантиметры в метры (6 см → 0.06 м)
    baseline_m = baseline_cm / 100.0
    
    # ========================================================================
    # БЛОК 4: КАЛИБРОВОЧНЫЙ КОЭФФИЦИЕНТ ДЛЯ КОРРЕКЦИИ СИСТЕМАТИЧЕСКИХ ОШИБОК
    # ========================================================================
    # Теоретическая формула глубины не учитывает все факторы:
    # - Погрешности калибровки камер
    # - Дисторсию линз (даже после коррекции)
    # - Масштабирование изображений при обработке
    # - Квантование диспаритета (16 уровней в SGBM)
    # 
    # Коэффициент подбирается экспериментально:
    # 1. Измерьте реальное расстояние до объекта линейкой/рулеткой
    # 2. Сравните с показанием на легенде
    # 3. CALIBRATION_SCALE = реальное_расстояние / показанное_расстояние
    # 
    # Пример: если рука на 0.45м показывает 0.12м → коэффициент = 0.45/0.12 = 3.75
    CALIBRATION_SCALE = 3.75
    
    # ========================================================================
    # БЛОК 5: ПАРАМЕТРЫ ДИСПАРИТЕТА ИЗ ГЛОБАЛЬНЫХ КОНСТАНТ
    # ========================================================================
    # NUM_DISP_BASE: максимальный диапазон диспаритета (320 = 16 × 20)
    # Задаётся в начале fused_depth_map.py, должно быть кратно 16 для OpenCV SGBM
    num_disparities = NUM_DISP_BASE
    
    # MIN_DISP_BASE: минимальный диспаритет (обычно 0 для ректифицированных изображений)
    # Увеличивается при работе с очень дальними объектами для экономии вычислений
    min_disparity = MIN_DISP_BASE
    
    # ========================================================================
    # БЛОК 6: ПАРАМЕТРЫ РАЗМЕРА И ПОЗИЦИИ ЛЕГЕНДЫ
    # ========================================================================
    # Ширина вертикального столбика легенды в пикселях
    legend_width = 55
    
    # Высота легенды: почти во всю высоту изображения минус отступы сверху/снизу
    legend_height = h - 40
    
    # X-координата левого края легенды: справа с отступом 10 пикселей от края
    legend_x = w - legend_width - 10
    
    # Y-координата верхнего края легенды: отступ 20 пикселей от верха
    legend_y = 20
    
    # ========================================================================
    # БЛОК 7: ДИНАМИЧЕСКОЕ ОПРЕДЕЛЕНИЕ ДИАПАЗОНА ГЛУБИН В ТЕКУЩЕМ КАДРЕ
    # ========================================================================
    # Фильтруем нулевые значения — это невалидные пиксели (нет данных о глубине)
    # Массив valid_depth содержит только пиксели с корректными измерениями
    valid_depth = fused_depth_uint8[fused_depth_uint8 > 0]
    
    # Вычисляем мин/макс значения глубины в текущем кадре для динамической шкалы
    if len(valid_depth) > 0:
        # norm_min: самое далёкое измеренное расстояние в кадре [0-255]
        # Ограничиваем минимум 1 для защиты от деления на ноль
        norm_min = max(1, np.min(valid_depth))
        
        # norm_max: самое близкое измеренное расстояние в кадре [0-255]
        # Ограничиваем минимум 1 для защиты от деления на ноль
        norm_max = max(1, np.max(valid_depth))
    else:
        # Резервные значения если в кадре нет валидных данных о глубине
        norm_min = 1
        norm_max = 255
    
    # ========================================================================
    # БЛОК 8: КОНВЕРТАЦИЯ НОРМАЛИЗОВАННЫХ ЗНАЧЕНИЙ [0-255] В ПИКСЕЛИ ДИСПАРИТЕТА
    # ========================================================================
    # fused_depth_uint8 хранит нормализованные значения 0-255 для визуализации
    # Для расчёта метров нужны реальные пиксели диспаритета из алгоритма SGBM
    # 
    # Формула: disparity_px = min_disp + (norm_value / 255) × num_disp
    # Пример: norm=128, num_disp=320 → disp = 0 + (128/255)×320 = 161 пикселей
    disp_min = min_disparity + (norm_min / 255.0) * num_disparities
    disp_max = min_disparity + (norm_max / 255.0) * num_disparities
    
    # Защита от деления на ноль в формуле триангуляции
    # Минимальный диспаритет 0.5 пикселя (физический предел для субпиксельных алгоритмов)
    disp_min = max(0.5, disp_min)
    disp_max = max(0.5, disp_max)
    
    # ========================================================================
    # БЛОК 9: КОНВЕРТАЦИЯ ДИСПАРИТЕТА В МЕТРЫ ПО ФОРМУЛЕ ТРИАНГУЛЯЦИИ
    # ========================================================================
    # Основная формула стереозрения:
    # depth_meters = (focal_length_pixels × baseline_meters) / disparity_pixels
    # 
    # Физический смысл:
    # - focal_length × baseline = константа системы (чем больше базис/фокус, тем точнее)
    # - disparity = измеренное смещение между камерами (больше = объект ближе)
    # 
    # max_depth_m: расстояние до самых далёких объектов (минимальный диспаритет)
    max_depth_m = ((focal_length * baseline_m) / disp_min) * CALIBRATION_SCALE
    
    # min_depth_m: расстояние до самых близких объектов (максимальный диспаритет)
    min_depth_m = ((focal_length * baseline_m) / disp_max) * CALIBRATION_SCALE
    
    # Ограничение диапазона для защиты от артефактов и выбросов
    # max_depth_m: максимум 30 метров (дальше точность стерео резко падает)
    max_depth_m = min(max_depth_m, 30.0)
    
    # min_depth_m: минимум 5 см (ближе камеры не могут сфокусироваться)
    min_depth_m = max(min_depth_m, 0.05)
    
    # ========================================================================
    # БЛОК 10: СОЗДАНИЕ ДИНАМИЧЕСКОГО ЦВЕТОВОГО ГРАДИЕНТА
    # ========================================================================
    # Создаём пустой массив для градиента размером с легенду (высота × ширина × 3 канала)
    # dtype=np.uint8 — стандартный формат изображений OpenCV (0-255)
    gradient_bar = np.zeros((legend_height, legend_width), dtype=np.uint8)
    
    # Заполняем градиент по вертикали: сверху=далеко(синий), снизу=близко(красный)
    for i in range(legend_height):
        # ratio: нормализованная позиция от 0.0 (верх) до 1.0 (низ)
        ratio = i / legend_height
        
        # gradient_value: значение глубины для текущей строки градиента
        # Линейная интерполяция между norm_min и norm_max текущего кадра
        gradient_value = norm_min + ratio * (norm_max - norm_min)
        
        # Запись значения в строку градиента с ограничением диапазона [0, 255]
        gradient_bar[i, :] = int(np.clip(gradient_value, 0, 255))
    
    # Применяем цветовую карту COLORMAP_JET для визуализации
    # 0 (далеко) → синий, 128 → зелёный/жёлтый, 255 (близко) → красный
    # Это та же карта что используется для основной карты глубины — цвета точно совпадают!
    gradient_colored = cv2.applyColorMap(gradient_bar, cv2.COLORMAP_JET)
    
    # Накладываем цветной градиент на изображение в позицию легенды
    img[legend_y:legend_y+legend_height, legend_x:legend_x+legend_width] = gradient_colored
    
    # ========================================================================
    # БЛОК 11: ОТРИСОВКА БЕЛОЙ РАМКИ ВОКРУГ ЛЕГЕНДЫ
    # ========================================================================
    # Рисуем прямоугольную рамку толщиной 1 пиксель для визуального выделения легенды
    # Цвет (255, 255, 255) = белый в BGR формате OpenCV
    cv2.rectangle(img, (legend_x, legend_y), 
                  (legend_x + legend_width, legend_y + legend_height), 
                  (255, 255, 255), 1)
    
    # ========================================================================
    # БЛОК 12: НАЛОЖЕНИЕ ТЕКСТОВЫХ МЕТОК НА ЛЕГЕНДУ
    # ========================================================================
    # Заголовок легенды над столбиком (жёлтый цвет для выделения)
    # Позиция: по центру над легендой, Y на 5 пикселей выше верхнего края
    cv2.putText(img, "МЕТРЫ", (legend_x - 70, legend_y - 5),
                cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 0), 1)
    
    # Метка "ДАЛЕКО" для верхней части легенды (синий цвет)
    # Позиция: слева от легенды, выровнена по верху
    cv2.putText(img, "ДАЛЕКО", (legend_x - 85, legend_y + 18),
                cv2.FONT_HERSHEY_COMPLEX, 0.4, (200, 50, 50), 1)
    
    # Числовое значение максимального расстояния (самые далёкие объекты в кадре)
    # Формат: 2 знака после запятой для точности (например, "8.45 м")
    # Позиция: внутри легенды, у верхнего края
    cv2.putText(img, f"{max_depth_m:.2f} м", (legend_x + 2, legend_y + 13),
                cv2.FONT_HERSHEY_COMPLEX, 0.35, (255, 255, 255), 1)
    
    # Метка "БЛИЗКО" для нижней части легенды (зелёный цвет)
    # Позиция: слева от легенды, выровнена по низу
    cv2.putText(img, "БЛИЗКО", (legend_x - 85, legend_y + legend_height - 5),
                cv2.FONT_HERSHEY_COMPLEX, 0.4, (50, 200, 50), 1)
    
    # Числовое значение минимального расстояния (самые близкие объекты в кадре)
    # Формат: 2 знака после запятой для точности (например, "0.42 м")
    # Позиция: внутри легенды, у нижнего края
    cv2.putText(img, f"{min_depth_m:.2f} м", (legend_x + 2, legend_y + legend_height - 8),
                cv2.FONT_HERSHEY_COMPLEX, 0.35, (255, 255, 255), 1)
    
    # Технические параметры для отладки и понимания значений (мелкий серый шрифт)
    
    # Диапазон диспаритета в пикселях: показывает реальные значения алгоритма SGBM
    # Формат: "d:15-180px" означает диспаритет от 15 до 180 пикселей в текущем кадре
    # Позиция: центр легенды по вертикали
    ...
    # Калибровочный коэффициент: показывает текущий множитель коррекции
    # Полезно при настройке — сразу видно если коэффициент изменён
    # Формат: "cal=3.75x" означает умножение глубины на 3.75
    # Позиция: ниже центра легенды
    cv2.putText(img, f"cal={CALIBRATION_SCALE:.2f}x", 
                (legend_x + 2, legend_y + legend_height//2 + 12),
                cv2.FONT_HERSHEY_COMPLEX, 0.25, (150, 150, 150), 1)
    
    # Базис системы в сантиметрах: расстояние между камерами из калибровки
    # Полезно для проверки что калибровка загрузилась корректно
    # Формат: "b=6.5см" означает базис 6.5 сантиметра
    # Позиция: ниже калибровочного коэффициента
    cv2.putText(img, f"b={baseline_cm:.1f}см", 
                (legend_x + 2, legend_y + legend_height//2 + 24),
                cv2.FONT_HERSHEY_COMPLEX, 0.25, (150, 150, 150), 1)
    
    # Возвращаем модифицированное изображение с наложенной легендой
    return img
